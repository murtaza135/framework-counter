...
export function extractRoutes(routes: Route[]) {
  const routesWithActions: RouteWithAction[] = routes
    .filter(({ path }) => path !== "*" && path.toLowerCase() !== "x")
    .map(route => ({ path: route.path, action: extractRouteResult(route) }));
  const notFoundRoute = routes.filter(({ path }) => path === "*")[0]?.html;
  const errorRoute = routes.filter(({ path }) => path.toLowerCase() === "x")[0]?.html;
